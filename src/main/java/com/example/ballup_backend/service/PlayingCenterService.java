...
public class PlayingCenterService {
        ...
        public List<PlayingCenterResponse> getCentersByOwnerId(Long ownerId) {
                ...
                return centers.stream().map(center -> {
                        List<String> imageUrls = playingCenterImageRepository.findByCenter(center).stream()
                                        ...
                                        .collect(Collectors.toList());
                        List<PlayingSlotResponse> slotResponses = playingSlotRepository.findSlotsByCenter(center)
                                        .stream()
        ...
        public PlayingCenterResponse getCenterInfo(Long id) {
                ...
                List<String> imageUrls = playingCenterImageRepository.findByCenter(playingCenter).stream()
                                .map(PlayingCenterImageEntity::getImage)
                                .collect(Collectors.toList());
                List<PlayingSlotResponse> slotResponses = playingSlotRepository.findByPlayingCenter(playingCenter)
                                .stream()
                                .map(slot -> PlayingSlotResponse.builder()
                                                .id(slot.getId())
                                                .name(slot.getName())
        ...
        public List<CardPlayingCenterResponse> getCenterByCriteria(String name, String address, Long fromDateTime,
                        Long toDateTime, String sortBy, String sortDirection, String sport) {
                ...
                if (fromDateTime != null && toDateTime != null) {
                        ...
                        List<Long> unavailableSlotIds = unavailableSlotRepository.findUnavailableSlots(fromTimestamp,
                                        toTimestamp);
                        System.out.println(unavailableSlotIds);

                        if (!unavailableSlotIds.isEmpty()) {
                                // Lấy danh sách centerId có slot bị chiếm
                                Map<Long, List<Long>> centerSlotMap = playingSlotRepository
                                                .findCentersBySlotIds(unavailableSlotIds)
                                                .stream()
                                                .map(result -> new AbstractMap.SimpleEntry<>((Long) result[0],
                                                                (Long) result[1]))
                ...
                return playingCenters.stream().map(center -> {
                        PlayingSlotEntity firstSlot = playingSlotRepository.findByPlayingCenter(center).stream()
        ...
        @Transactional
        public void deletePlayingCenter(Long id) {
                PlayingCenterEntity center = playingCenterRepository.getReferenceById(id);
                if (!playingCenterRepository.existsById(id)) {
                        throw new EntityNotFoundException("Playing center not found!");
                }
                List<PlayingSlotEntity> slots = playingSlotRepository.findByPlayingCenterId(id);
                playingSlotRepository.deleteAll(slots);
                playingCenterImageRepository.deleteByPlayingCenter(center);
